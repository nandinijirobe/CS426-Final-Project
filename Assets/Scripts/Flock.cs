...
public class Flock : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!b.Contains(transform.position)) { // checks if NPC is within the bounding box around FM
            turning = true;
        } else {
            turning = false;
        }
    ...
    void ApplyRules() {
        ...
        foreach (GameObject go in gos) { // Iterate through all NPCs in the group 
            if (go != this.gameObject) { // Ignore self
                ...
                if (nDistance <= FlockManager.FM.neighbourDistance) { // Check if it is within a neighbourly distance
        ...
        if (groupSize > 0) {
            ...
            if (direction != Vector3.zero) { // rotate and move towards new position
                transform.rotation = Quaternion.Slerp(transform.rotation,
                                                      Quaternion.LookRotation(direction),
                                                      FlockManager.FM.rotationSpeed * Time.deltaTime);
